...
function doPost(e) {
  try {
    ...
    // 세 번째 시트(회원 정보)에서 이름·학번 일치 시에만 대여 허용
    // ---------------------------
    const name = (data.name || "").trim();
    const studentId = String(data.studentId || "").trim();
    ...
    var memberSheet = allSheets[2];
    var memberValues = memberSheet.getDataRange().getValues();
    var memberRows = memberValues.length > 1 ? memberValues.slice(1) : [];
    var nameMatch = false;
    for (var i = 0; i < memberRows.length; i++) {
      var rowName = String(memberRows[i][0] || "").trim();
      var rowStudentId = String(memberRows[i][1] || "").trim();
    ...
    if (!nameMatch) {
      return ContentService.createTextOutput(
        JSON.stringify({
          success: false,
          message: "이름과 학번이 등록된 정보와 일치하지 않습니다. 대여할 수 없습니다."
        })
      ).setMimeType(ContentService.MimeType.JSON);
    }
