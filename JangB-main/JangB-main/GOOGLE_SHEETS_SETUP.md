...
4. **장비 반납 시 본인 확인용** 스프레드시트에는 **세 번째 시트**를 추가하고, 회원 정보를 입력합니다.
   - 시트 이름은 자유 (예: 시트3, 회원목록 등).
   - A열: 이름, B열: 학번 (예: 2023-16142).
   - 첫 번째 행은 헤더(이름, 학번)로 두고, 2행부터 데이터를 입력합니다.
   - 반납 시 입력한 이름·학번이 이 시트의 어떤 행과 일치할 때만 반납이 처리됩니다.
